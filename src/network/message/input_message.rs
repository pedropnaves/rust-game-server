...
impl InputMessage {
        ...
        match read_result {
            Ok(_) => {
                ...
                reader
                    .by_ref()
                    .take(length as u64)
                    .read_exact(&mut buffer[Message::HEADER_LENGTH..])
                    .unwrap();
    ...
    pub fn get_string(&mut self) -> std::string::String {
        ...
        self.message
            .buffer
            .read_with::<&str>(&mut offset, Str::Len(size))
            .unwrap()
            .to_string()
    }
}
