...
use crate::network::connection::connection::MessageSender;
...
impl <T : MessageSender> Protocol<T> {
    ...
    pub fn handle_disconnect(&mut self) {
        ...
        let mut output_message = self.protocol_serializer.create_notify_player_disconnected(&player.clone().unwrap());
        self.player = None;
        GameWorld::get_instance().lock().unwrap().disconnect_player(&player.unwrap());
        self.message_sender.send_message_to_everyone(&mut output_message);
    }
    ...
    fn do_login_for_player(&mut self, input_message: &mut InputMessage)  {
        ...
        output_message = self.protocol_serializer.create_notify_player_connected(&new_player);
        self.message_sender.send_message_to_everyone(&mut output_message);
    }
